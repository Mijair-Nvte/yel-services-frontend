...
// 1. Interfaz adaptada para Seguros
interface InsuranceSettings {
  notifications: {
    notify_on_new_insurance: boolean;
...
const defaultInsuranceSettings: InsuranceSettings = {
  notifications: {
    notify_on_new_insurance: false,
...
export default function InsuranceSettingsTab({ workspaceUid }: InsuranceTabProps) {
  // 3. Usamos el Hook pidiéndole el módulo 'insurances' (Como lo declaramos en el Job de Laravel)
  const {
    settings,
    setSettings,
    isLoading,
    ...
    error,
    saveSettings,
  } = useCompanySettings<InsuranceSettings>(workspaceUid, 'insurances', defaultInsuranceSettings);
      ...
      try {
        ...
        const filteredUsers = rawMembers
          .map((member: any) => {
            ...
            // Filtramos a los que sí deben poder recibir notificaciones administrativas
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-16 text-muted-foreground gap-2">
        ...
        <span>Cargando configuración de seguros...</span>
      </div>
    );
  }
  ...
  return (
    <Card className="border shadow-sm">
      <CardHeader>
        ...
        <CardDescription>
          Administra las alertas automáticas y conexiones externas para los registros de seguros.
        </CardDescription>
      </CardHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-4 pl-6">
            <div className="flex items-center space-x-3">
              <Checkbox
                id="notify_new_insurance"
                checked={settings.notifications.notify_on_new_insurance}
                onCheckedChange={(checked) => 
                  setSettings({
                    ...settings,
                    notifications: { ...settings.notifications, notify_on_new_insurance: checked === true }
                  })
                }
              />
              <Label htmlFor="notify_new_insurance" className="text-sm font-normal cursor-pointer leading-none">
                Notificar al equipo cuando ingrese un nuevo prospecto de seguro
              </Label>
            </div>
            ...
            {settings.notifications.notify_on_new_insurance && (
          ...
          <div className="grid gap-4 pl-6">
            <div className="flex items-center space-x-3">
              ...
              <Label htmlFor="webhook_active" className="text-sm font-normal cursor-pointer leading-none">
                Activar Webhook al registrar un seguro
              </Label>
            </div>
            ...
            {settings.integrations.webhook_active && (
              <div className="space-y-2 pt-2 animate-in fade-in-50">
                ...
                <p className="text-xs text-muted-foreground">
                  Esta URL recibirá un POST con los datos del seguro para detonar workflows de automatización.
                </p>
              </div>
            )}
          </div>
