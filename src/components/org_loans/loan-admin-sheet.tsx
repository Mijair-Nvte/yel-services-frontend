...
export function LoanAdminSheet({
    open,
    onOpenChange,
    application,
    onUpdate,
    ...
    useEffect(() => {
        if (open && application) {
            setStatus((application.status as AppStatus) || "Open");
            ...
            <SheetContent className="sm:max-w-md overflow-y-auto p-0 flex flex-col bg-white border-l shadow-2xl">
                <SheetHeader className="space-y-1 border-b p-6 pb-4">
                    ...
                    <SheetTitle className="text-xl flex items-center justify-between">
                        <span>Gestión de Préstamo</span>
                        <Button
                            variant="outline"
                            onClick={async () => {
                                if (window.confirm("¿Estás seguro de eliminar esta solicitud de préstamo? Esta acción es irreversible.")) {
                        ...
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1 space-y-1">
                                ...
                                <div className="flex items-center gap-2 group">
                                    ...
                                    {displayEmail && <CopyButton text={displayEmail} />}
                                </div>

                                <div className="flex items-center gap-2 group">
                                    <div className="flex items-center gap-1.5 text-slate-500 text-sm">
                                        <Phone className="h-3.5 w-3.5 text-slate-400" />
                                        <span>{displayPhone || "Sin teléfono"}</span>
                                    </div>
                                    {displayPhone && <CopyButton text={displayPhone} />}
                                </div>
                            ...
                            <div className="relative z-10 space-y-4">
                                ...
                                <div className="flex justify-between items-end border-t border-white/20 pt-4">
                                    ...
                                    <div className="text-right">
                                        <Landmark className="h-10 w-10 opacity-20 inline-block mb-1" />
                                    </div>
                                </div>
                            </div>
                        </div>
